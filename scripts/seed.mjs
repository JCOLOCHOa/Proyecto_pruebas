...
envContent.split('\n').forEach(line => {
  ...
  if (line.startsWith('SUPABASE_KEY=')) supabaseKey = line.split('=')[1].replace(/"/g, '').trim();
});

const supabase = createClient(supabaseUrl, supabaseKey);

import { productsData } from './data/products.js'; // Let's just inline the products to avoid TS/JS module issues in bare Node.
...
async function seed() {
  ...
  if (error) {
    ...
    console.log('Seed terminado. Productos insertados:', data.length);
  }
}
